...
func main() {
	...
	userId, err := AddUser(&user, tx, ctx)
	if err != nil {
		tx.Rollback()
		log.Fatal(err)
	}

	err = AddProfile(userId, tx, ctx)
	if err != nil {
		return
	}
...
func AddUser(user *User, tx *sql.Tx, ctx context.Context) (int64, error) {
	stmt, err := tx.PrepareContext(ctx, "INSERT INTO users(username, email, password) values(?, ?, ?)")
...
func AddProfile(userId int64, tx *sql.Tx, ctx context.Context) error {
	stmt, err := tx.PrepareContext(ctx, "INSERT INTO profiles(user_id, balance) values(?, ?)")
	if err != nil {
		return err
	}
